...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await BorderBuddyService.getStatus(id, user.id)
  if (!result.ok) {
    ...
    return responses.serverError('Unable to check BorderBuddy status')
  }

  return ok({ enabled: result.enabled, borderBuddy: result.borderBuddy })
}
